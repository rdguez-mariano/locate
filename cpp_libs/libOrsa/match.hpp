...
struct Match {
    ...
    : x1(ix1), y1(iy1), x2(ix2), y2(iy2), similarity(1.0) {}
    Match(float ix1, float iy1, float ix2, float iy2, float similarity)
    : x1(ix1), y1(iy1), x2(ix2), y2(iy2), similarity(similarity) {}
    float x1, y1, x2, y2, similarity;
    int Qidx, Tidx;
    ...
    */
    ...
    // Match operator-(const Match& in) const {
    // return Match(this->x1-in.x1, this->y1-in.y1, this->x2-in.x2, this->y2-in.y2);
    // }
